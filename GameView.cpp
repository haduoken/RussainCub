...
GameView::GameView() : UpCube(Cube::CubeType(random() % 6), random() % 4, 20, 20) {
  Startview = Interface();
  DownView = Interface();
  UpView = Interface();
  DownCube = DownCubes();
...
bool GameView::IsCollission(Cube &cube) {
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 5; j++) {
      if (cube.Display[j][i]) {
        if (2 * i + cube.locate_x < 12 || 2 * i + 2 + cube.locate_x > 40) return true;
  ...
  for (int i = 0; i < 30; i++)
    for (int j = 0; j < 40; j++)
      if (DownCube.BackGround[i][j]) {
        p.PrintXY("1", TerminalControl::ColorType::RED, TerminalControl::ColorLocate::BACK, j + 60, i + 2);
      } else {
        p.PrintXY("0", TerminalControl::ColorType::RED, TerminalControl::ColorLocate::BACK, j + 60, i + 2);
      }
}
  ...
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 5; j++) {
      if (cube.Display[j][i]) {
        ...
        DownCube.BackGround[cube.locate_y + j][cube.locate_x + 2 * i + 1] = true;
      }
    }
...
bool GameView::Act(char ch) {
  switch (ch) {
    case 'w': {
      Cube NextCube_mode =
          Cube(UpCube.type_, (UpCube.CubeMode_ + 1) % UpCube.maxCubeMode_, UpCube.locate_x, UpCube.locate_y);
      if (!IsCollission(NextCube_mode)) {
        UpView.ClearArea(UpCube.locate_x, UpCube.locate_y, 5, 5, UpCube.Display);
        UpView.InserArea(NextCube_mode.locate_x, NextCube_mode.locate_y, 5, 5, NextCube_mode.Display);
    ...
    case 's': {
      Cube NextCube_down = Cube(UpCube.type_, UpCube.CubeMode_, UpCube.locate_x, UpCube.locate_y + 1);
      if (!IsCollission(NextCube_down)) {
        UpView.ClearArea(UpCube.locate_x, UpCube.locate_y, 5, 5, UpCube.Display);
        UpView.InserArea(NextCube_down.locate_x, NextCube_down.locate_y, 5, 5, NextCube_down.Display);
    ...
    case 'a': {
      Cube NextCube_left = Cube(UpCube.type_, UpCube.CubeMode_, UpCube.locate_x - 2, UpCube.locate_y);
      if (!IsCollission(NextCube_left)) {
        UpView.ClearArea(UpCube.locate_x, UpCube.locate_y, 5, 5, UpCube.Display);
        UpView.InserArea(NextCube_left.locate_x, NextCube_left.locate_y, 5, 5, NextCube_left.Display);
    ...
    case 'd': {
      Cube NextCube_right = Cube(UpCube.type_, UpCube.CubeMode_, UpCube.locate_x + 2, UpCube.locate_y);
      if (!IsCollission(NextCube_right)) {
        UpView.ClearArea(UpCube.locate_x, UpCube.locate_y, 5, 5, UpCube.Display);
        UpView.InserArea(NextCube_right.locate_x, NextCube_right.locate_y, 5, 5, NextCube_right.Display);
